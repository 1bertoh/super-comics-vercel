...
export default function Author() {
    ...
    const getInput = async ({target}) => {
        ...
        const res = await a.GET({urlName:url});
        const resJson = await res.json();
        const authors = resJson.message
        setIsUrlOk(!Boolean(authors.length))
    }
    ...
    const submit = async (e) => {
        ...
        try {
            const a = new AuthorAPI();
            const jsonA = JSON.stringify({ name: name, urlName });

            const response = await a.POST(jsonA);
            const responseJson = await response.json()

            const resObj = {ok: response.ok, status: response.status, body: responseJson };
